...
pub const Song = struct {
    ...
    pub fn init(allocator: Allocator, database: DB, id: ID) !Self {
        ...
        const query = try std.fmt.allocPrint(
            allocator,
            "SELECT title,album,artist,year,path FROM songs WHERE ID={}",
            .{id}
        );
        ...
        return Self {
            .id = id,
            .title = try allocator.dupe(u8, data[0]),
            .album = if (data[1]) |album| try allocator.dupe(u8, album) else null,
            .artist = try allocator.dupe(u8, data[2]),
            .year = data[3],
            .path = try allocator.dupe(u8, data[4]),
            .updated = false,
            .allocator = allocator
        };
    }
...
pub const App = struct {
    ...
    pub fn init(allocator: Allocator) !Self {
        return Self {
            .queue = std.ArrayList(ID).init(allocator),
            .db = try DB.init(consts.DATABASE_PATH),
            .playlists = std.ArrayList(Playlist).fromOwnedSlice(
                allocator,
                try playlist.loadPlaylists(allocator, consts.PLAYLISTS_JSON_PATH)
            ),
            .allocator = allocator
        };
    }
    ...
    pub fn querySongs(self: Self, str: []const u8) []Song {
        ...
        const query = try std.fmt.allocPrint(
            self.allocator,
            "SELECT id,title,album,artist,year,path FROM songs WHERE {s}",
            .{str}
        );
        ...
        return Self {
            .id = data[0],
            .title = try self.allocator.dupe(u8, data[1]),
            .album = if (data[2]) |album| try self.allocator.dupe(u8, album) else null,
            .artist = try self.allocator.dupe(u8, data[3]),
            .year = data[4],
            .path = try self.allocator.dupe(u8, data[5]),
            .updated = false,
