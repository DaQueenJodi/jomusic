...
pub const MetadataConfig = struct {
    ...
    pub fn fromMetadata(metadata: Self) !MetadataConfig {
        return MetadataConfig{ .title = metadata.title orelse return error.MissingTitle, .artist = metadata.artist orelse return error.MissingArtist, .album = metadata.album, .year = metadata.year, .path = metadata.path };
    }
};
...
pub fn init(allocator: Allocator, path: [:0]const u8) !Self {
    ...
    return Self{ .allocator = allocator, .title = try c_str_to_optional(allocator, c.taglib_tag_title(tag)), .artist = try c_str_to_optional(allocator, c.taglib_tag_artist(tag)), .album = try c_str_to_optional(allocator, c.taglib_tag_album(tag)), .year = c.taglib_tag_year(tag), .path = path };
}
...
pub fn format(self: Self, comptime _: []const u8, _: fmt.FormatOptions, out: anytype) !void {
    try out.print(
        \\Metadata {{ 
        \\  title: {s},
        \\  album: {s},
        ...
        \\  year: {}
        \\}}
    , .{ self.title orelse "None", self.album orelse "None", self.artist orelse "None", self.year });
}
